...
var _default = options => {
  ...
  return (// @ts-ignore
    (_Observable$create = _rxjs.Observable.create(observer => {
    ...
    , (0, _operators.publish)()(_Observable$create)). // @ts-ignore
    refCount()
  );
};
...
const countLines = str => (0, _stripAnsi.default)(str).split('\n').reduce((p, line) => process.stdout.columns ? p + Math.ceil((line.length || 1) / process.stdout.columns) : p + 1, 0); // Pad a string with spaces on either side
...
var _default = async (packages, path = __dirname.includes('node_modules') ? _path.default.resolve(__dirname, '..', '..') : __dirname) => typeof packages === 'string' ? localRequire(packages, path) : Promise.all(packages.map(pkg => localRequire(pkg, path)));
...
var _default = options => _rxjs.Observable.create(observer => {
  if (!pptr) pptr = (0, _localRequire.default)('puppeteer');
  let closed;
  ...
  browser.then(browser => {
    ...
    observer.next(_rxjs.Observable.create(observer => {
      ...
      const emptyHTMLFilePath = _path.default.resolve(__dirname, '..', 'dist', 'empty.html');

      const url = (0, _utils.transformPathToEpkUrl)(emptyHTMLFilePath, options.port);
      page.then(page => {
        observer.next({
          loadFile: file => page.goto(url).then(() => page.addScriptTag({
            url: file.url
          })),
...
var _default = options => (0, _operators.switchMap)(val => _rxjs.Observable.create(observer => {
  ...
  if (!imports) imports = (0, _localRequire.default)(['koa', 'koa-static', 'koa-mount']);
...
var _bundler5 = _interopRequireDefault(require("./bundler"));
...
var _default = _options => {
  ...
  if (!_utils.isBrowser && options.target === _types.BUNDLER_TARGET.BROWSER) {
    _port = (0, _localRequire.default)('get-port').then(getPort => getPort({
      port: 10485
    })).then(port => options.port = port);
  } // @ts-ignore


  return _ref = (_ref2 = (_of = (0, _rxjs.of)([(0, _bundler5.default)(options), (!_utils.isBrowser && options.target === _types.BUNDLER_TARGET.BROWSER ? options.browsers : [options.target]).map(target => (0, _targetRuntimeProvider.default)(target, options))]) // @ts-ignore
  , (!_utils.isBrowser && options.target === _types.BUNDLER_TARGET.BROWSER ? (0, _operators.delayWhen)(() => (0, _rxjs.from)(_port)) : (0, _operators.tap)())(_of) // @ts-ignore
    ...
    var _bundler, _bundler2, _merge;

    return (0, _rxjs.merge)(( // @ts-ignore
    _bundler = bundler, (0, _operators.filter)(({
      name
    }) => name === 'buildStart')(_bundler)), ( // @ts-ignore
    _bundler2 = bundler, (0, _operators.filter)(({
      name
    }) => name === 'bundled')(_bundler2)), ( // @ts-ignore
    _merge = (0, _rxjs.merge)(...targetRuntimeProviders) // @ts-ignore
    , (0, _operators.mergeMap)((targetRuntimeProvider // @ts-ignore
    ) => {
      var _ref3, _ref4, _bundler3;

      return _ref3 = (_ref4 = (_bundler3 = bundler // @ts-ignore
      , (0, _operators.filter)(({
        name
      }) => name === 'buildStart')(_bundler3) // @ts-ignore
        ...
        return (// @ts-ignore
          _ref5 = (_bundler4 = bundler // @ts-ignore
          , (0, _operators.filter)(({
            name
          }) => name === 'bundled')(_bundler4) // @ts-ignore
        ...
        return (// @ts-ignore
          _from = (0, _rxjs.from)(bundle.isEmpty ? Array.from(bundle.childBundles) : [bundle]) // @ts-ignore
          , (0, _operators.mergeMap)(childBundle => {
            var _of2, _ref6, _newContextObservable, _ref7, _ref8, _analyzedObservable;

            const {
              name: path
            } = childBundle; // @ts-ignore
...
if (!module.parent) require("_bundle_loader")(require.resolve('./cli/index')).then(() => {}); // keep the then because parcel lazy-load imports
