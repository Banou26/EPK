...
exports.transformPathToEpkUrl = exports.transformPathToTestUrl = exports.prettifyPath = void 0;
...
const transformPathToTestUrl = (path, port) => `${port ? `http://localhost:${port}` : ''}${path.replace(`${_path.default.resolve(process.cwd(), '.epk', 'dist')}\\`, '/tests/').replace('\\', '/')}`;

exports.transformPathToTestUrl = transformPathToTestUrl;

const transformPathToEpkUrl = (path, port) => `${port ? `http://localhost:${port}` : ''}${path.replace(`${_path.default.resolve(process.cwd(), 'dist')}\\`, '/epk/').replace('\\', '/')}`;
...
var _default = options => _rxjs.Observable.create(observer => {
  ...
  browser.then(browser => {
    ...
    observer.next(_rxjs.Observable.create(observer => {
      ...
      page.then(page => {
        observer.next({
          loadFile: file => page.goto((0, _utils.transformPathToUrl)('/epk/empty.html', options.port)).then(() => page.evaluate()),
...
var _default = (targetRuntimeProvider, options) => (0, _operators.switchMap)(file => {
  ...
  return (// @ts-ignore
    ...
    , (0, _operators.mergeMap)(async targetRuntime => {
      await targetRuntime.loadFile(file); // console.log(analyzeStr(options, { type: MESSAGE_TYPE.GET_TESTS }))

      const tests = await targetRuntime.exec(analyzeStr(options, {
...
var _default = _options => {
  ...
  return _ref = (_ref2 = (_of = (0, _rxjs.of)([(0, _bundler4.default)(options), (!_utils.isBrowser && options.target === _types.BUNDLER_TARGET.BROWSER ? options.browsers : [options.target]).map(target => (0, _targetRuntimeProvider.default)(target, {}))]) // @ts-ignore
      ...
      return (// @ts-ignore
        ...
        , (0, _operators.mergeMap)((targetRuntimeProvider // @ts-ignore
          ...
          , (0, _operators.mergeMap)(path => {
            var _of2, _ref6, _newContextObservable, _ref7, _analyzedObservable;
            ...
            _ref7 = (_analyzedObservable = analyzedObservable // @ts-ignore
            , (0, _test.default)(targetRuntimeProvider, options)(_analyzedObservable) // @ts-ignore
...
if (!module.parent) require("_bundle_loader")(require.resolve('./cli/index')).then(() => {});
