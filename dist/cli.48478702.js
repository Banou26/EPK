...
const formatErrors = files => `${_chalk.default.reset.red(`Errors:`)}
...
${_chalk.default.reset.bold.gray(`Files:`)}
...
${files.map(({
  ...
  return _chalk.default.reset[!isFinished ? 'gray' : hasErrors ? 'red' : 'green'](`${(0, _utils.prettifyPath)(name)} ${finishedTests.length}(${finishedTests.length - erroredTests.length})/${tests.length}`);
}).map(str => `  ${str}`).join('\n')}`;
...
var _default = options => (0, _operators.scan)((state, val) => {
  ...
  _logger.default[isFinished ? 'success' : 'progress'](format(files));
