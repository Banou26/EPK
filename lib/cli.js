  ...
  , operators.mergeMap(runtimeProvider => {
    ...
    , operators.switchMap(testBundle => {
      ...
      , operators.mergeMap(testFile => {
        ...
        , operators.filter(() => testFile.hashes.size && hashDifference.size)(_of) // @ts-ignore
        ...
        , operators.filter(() => testFile.hashes.size && !hashDifference.size)(_of2))) // @ts-ignore
        , operators.switchMap((testFile // @ts-ignore
        ) => {
          var _from3;
...
var Reporter = (({
  ...
  const [tabsElement] = useTabs({
    stdin,
    setRawMode
  }, aggregatedTestFiles);
  return React__default.createElement(ink.Box, null, tabsElement, error && React__default.createElement(ink.Color, {
    red: true
