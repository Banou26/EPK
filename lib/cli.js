...
var spinners = _interopDefault(require('cli-spinners'));
...
var Spinner = (({
  ...
  React.useEffect(() => {
    const timer = setInterval(() => setFrame(frame === spinner.frames.length - 1 ? 0 : frame + 1), spinner.interval);
...
var File = (({
  testFile: testFileAggregation
}) => {
  var _testFileAggregation$, _testFileAggregation$2;

  const tests = testFileAggregation === null || testFileAggregation === void 0 ? void 0 : testFileAggregation.tests;
  const runtimeTestFiles = Array.from(testFileAggregation.testFiles.values());
  const testedTestsArray = ((_testFileAggregation$ = testFileAggregation.tests) === null || _testFileAggregation$ === void 0 ? void 0 : _testFileAggregation$.filter(({
    description
  }) => runtimeTestFiles.every(testFiles => {
    var _testFiles$tests;

    return (_testFiles$tests = testFiles.tests) === null || _testFiles$tests === void 0 ? void 0 : _testFiles$tests.some(({
      description: _description,
      executionEnd
    }) => _description === description && executionEnd);
  }))) || [];
  const isPreprocessed = testFileAggregation.tests;
  const testedTest = testedTestsArray.length;
  const totalTests = ((_testFileAggregation$2 = testFileAggregation.tests) === null || _testFileAggregation$2 === void 0 ? void 0 : _testFileAggregation$2.length) || 0; // todo: replace green by greenBright when ink will fix it

  return React__default.createElement(ink.Box, null, React__default.createElement(ink.Color, {
    rgb: isPreprocessed && testedTest === totalTests && brightGreen
  }, prettifyPath(testFileAggregation.name), " ", isPreprocessed ? `${testedTest}/${totalTests}` : React__default.createElement(Spinner, null)));
});
