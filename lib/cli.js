...
var Tester = (options => rxjs.Observable.create(observer => {
  ...
  , operators.mergeMap(runtimeProvider => {
    var _ref11, _ref12, _bundle;

    return _ref11 = (_ref12 = (_bundle = bundle // @ts-ignore
    , operators.mergeScan(([acc], testBundle // @ts-ignore
    ) => {
      var _ref13, _of;

      return _ref13 = (_of = rxjs.of(testBundle) // @ts-ignore
      , operators.map(value => {
        return [acc, value];
      })(_of) // @ts-ignore
      ), operators.takeUntil(bundle)(_ref13);
    }, [])(_bundle) // @ts-ignore
    ), operators.map(([, value]) => value)(_ref12) // @ts-ignore
      ...
      return _ref14 = (_from2 = rxjs.from(testFiles) // @ts-ignore
      , operators.scan((testFiles, testFile) => testFiles.set(testFile.name, testFile), new Map())(_from2) // @ts-ignore
      ), operators.map(testFiles => [runtimeProvider.runtimeName, testFiles])(_ref14);
    })(_ref11);
