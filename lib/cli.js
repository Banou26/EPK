...
var test = (test => operators.mergeMap(({
  loadFile,
  inMessages,
  outMessages
}) => AsyncObservable(async observer => {
  ...
  ), pluck('test')(_ref3) // @ts-ignore
  ), operators.take(1)(_ref2) // @ts-ignore
  ), startWith(undefined)(_ref));
  const logs = ( // @ts-ignore
  _ref4 = (_ref5 = (_ref6 = (_ref7 = (_inMessages2 = inMessages // @ts-ignore
  , takeUntil((_tests = tests, skip(1)(_tests)))(_inMessages2) // @ts-ignore
  ), filter(({
    type
  }) => type === MESSAGE.LOG)(_ref7) // @ts-ignore
  ), pluck('log')(_ref6) // @ts-ignore
  ), scan((arr, log) => [...arr, log], [])(_ref5) // @ts-ignore
  ), startWith([])(_ref4));
  const result = (_ref8 = (_ref9 = (_tests2 = tests // @ts-ignore
  , operators.take(2)(_tests2) // @ts-ignore
  ), combineLatest(logs)(_ref9) // @ts-ignore
  ), map(([testResult, logs]) => ({
    parcelBundle: test.parcelBundle,
    bundle: test.bundle,
    analyzedTestFile: test.analyzedTestFile,
    test,
    type: testResult === null || testResult === void 0 ? void 0 : testResult.type,
    value: testResult === null || testResult === void 0 ? void 0 : testResult.value,
    logs: logs,
    timeStart: testResult === null || testResult === void 0 ? void 0 : testResult.timeStart,
    timeEnd: testResult === null || testResult === void 0 ? void 0 : testResult.timeEnd
  }))(_ref8)); // @ts-ignore

  result.subscribe(testResult => observer.next(testResult), err => observer.error(err), () => observer.complete());
  outMessages.next({
...
var analyze = (() => operators.mergeMap(testResult => {
  return;
}));

var Tester = (options => rxjs.Observable.create(observer => {
  var _ref, _Parcel, _ref2, _ref3, _ref4, _parcel, _ref7, _from, _ref8, _runtimeProvider;
  ...
  , operators.mergeMap(runtimeProvider => {
    ...
    , operators.switchMap(testBundle => {
      ...
      return _ref9 = (_ref10 = (_from2 = rxjs.from(testFiles) // @ts-ignore
      , operators.mergeMap(testFile => {
        var _ref11, _ref12, _ref13, _of, _ref15, _ref16, _merge, _preprocessed, _of2, _ref19, _merge2, _ref20, _of3, _merge3;
        ...
        _ref11 = (_ref12 = (_ref13 = (_of = rxjs.of(testFile) // @ts-ignore
        , operators.filter(() => hashDifference.size)(_of) // @ts-ignore
        ), operators.switchMap((testFile // @ts-ignore
        ) => {
          var _ref14, _runtimeProvider2;

          return _ref14 = (_runtimeProvider2 = runtimeProvider // @ts-ignore
          , preprocessor(testFile)(_runtimeProvider2) // @ts-ignore
          ), operators.takeWhile(({
            preprocessingEnd
          }) => !preprocessingEnd, true)(_ref14);
        })(_ref13) // @ts-ignore
        ), operators.tap(testFile => testFileCache.set(name, testFile))(_ref12) // @ts-ignore
        ), operators.share()(_ref11));
        const tested = ( // @ts-ignore
        _ref15 = (_ref16 = (_merge = rxjs.merge((_preprocessed = preprocessed // @ts-ignore
        , operators.filter(({
          preprocessingEnd
        ...
        , operators.switchMap((testFile // @ts-ignore
          ...
          , operators.mergeMap((_test // test hasn't changed and is in the cache
            ...
            return _ref17 = (_ref18 = !hashDifference.size && cachedTestFile.tests.find(test => test.description === _test.description && 'executionEnd' in test) || // test has changed or wasn't in the cache
            // @ts-ignore
            runtimeProvider // @ts-ignore
            , test(_test)(_ref18) // @ts-ignore
            ), operators.takeWhile(({
              executionEnd
            }) => !executionEnd, true)(_ref17);
          })(_from3);
        })(_merge) // @ts-ignore
        ), updateCache(_ref16) // @ts-ignore
        ), operators.share()(_ref15));
        const analyzed = ( // @ts-ignore
        _ref19 = (_merge2 = rxjs.merge(tested, ( // @ts-ignore
        _ref20 = (_of3 = rxjs.of(cachedTestFile) // @ts-ignore
        , operators.filter(() => !hashDifference.size)(_of3) // @ts-ignore
        ), operators.mergeMap(({
          tests
        }) => rxjs.from(tests))(_ref20))) // @ts-ignore
        , operators.mergeMap((test // test hasn't changed and is in the cache
        ) => {
          var _ref21, _ref22;

          return _ref21 = (_ref22 = !hashDifference.size && cachedTestFile.tests.find(test => test.description === _test.description && 'analyzeEnd' in test) || // @ts-ignore
          runtimeProvider // @ts-ignore
          , analyze(test)(_ref22) // @ts-ignore
          ), operators.takeWhile(({
            analyzeEnd
          }) => !analyzeEnd, true)(_ref21);
        })(_merge2) // @ts-ignore
        ), updateCache(_ref19)); // @ts-ignore

        return _merge3 = rxjs.merge(rxjs.of(hashDifference.size ? testFile : cachedTestFile), preprocessed, tested, analyzed) // @ts-ignore
        , operators.map(() => testFileCache.get(name))(_merge3);
      })(_from2) // @ts-ignore
      ), operators.scan((testFiles, testFile) => testFiles.set(testFile.name, testFile), new Map())(_ref10) // @ts-ignore
      ), operators.map(testFiles => [runtimeProvider.runtimeName, testFiles])(_ref9);
    })(_bundle);
...
var File = (file => React.createElement(Box, null, file.name));
...
var Reporter = (({
  ...
  return React$1__default.createElement(ink.Box, null, state.files.length && React$1__default.createElement(ink.Box, null, state.files.map(file => React$1__default.createElement(File, {
    file: file
  }))), error && React$1__default.createElement(ink.Color, {
    red: true
  }, React$1__default.createElement(ink.Box, {
    flexDirection: "column"
  }, React$1__default.createElement(ink.Box, {
    padding: 1
  }, "An internal error with EPK happened, you should probably report this error here https://github.com/FKN48/EPK/issues"), React$1__default.createElement(ink.Box, null, error.stack))));
});

var CLIReporter = (subject => ink.render(React$1__default.createElement(Reporter, {
  subject: subject
})));
