...
var preprocessTestFile = (testFile => operators.mergeMap(({
  loadFile,
  inMessages,
  outMessages
}) => AsyncObservable(async observer => {
...
var test$1 = (unsubscribe => {
  var _mergeScan;

  return (// @ts-ignore
    _mergeScan = operators.mergeScan(([cache], testFile) => {
      var _ref, _ref2, _ref3, _of, _ref5, _ref6, _merge, _preprocessedTestFile, _of2, _merge2, _testResult, _of3, _ref11, _mergedResult;
      ...
      const hasChanged = hashDifference.size || hashes.size;
      const preprocessedTestFile = ( // @ts-ignore
      _ref = (_ref2 = (_ref3 = (_of = rxjs.of(testFile) // @ts-ignore
      , filter(() => hasChanged)(_of) // @ts-ignore
      ), switchMap(() => {
        var _ref4, _runtimeProvider;

        return (// @ts-ignore
          _ref4 = (_runtimeProvider = runtimeProvider // @ts-ignore
          , preprocessTestFile(testFile)(_runtimeProvider) // @ts-ignore
          ), takeWhile(({
            preprocessingEnd
          }) => !preprocessingEnd, true)(_ref4)
        );
      })(_ref3) // @ts-ignore
      ), tap(({
        tests,
      ...
      const testResult = ( // @ts-ignore
      _ref5 = (_ref6 = (_merge = rxjs.merge((_preprocessedTestFile = preprocessedTestFile // @ts-ignore
      , filter(({
        preprocessingEnd
      }) => preprocessingEnd)(_preprocessedTestFile)), ( // @ts-ignore
      _of2 = rxjs.of(cachedTestFile) // @ts-ignore
      , filter(() => !hashDifference.size && testFile.hashes.size)(_of2))) // @ts-ignore
      , switchMap((testFile // @ts-ignore
      ) => {
        var _from;

        return _from = from(testFile.tests) // @ts-ignore
        , mergeMap((test // test hasn't changed and is in the cache
        ) => {
          var _ref7, _ref8;

          return _ref7 = (_ref8 = !hashDifference.size && cachedTestFile.tests.find(_test => test.description === _test.description && 'executionEnd' in _test) || // test has changed or wasn't in the cache
          // @ts-ignore
          runtimeProvider // @ts-ignore
          , run(testFile, test)(_ref8) // @ts-ignore
          ), takeWhile(({
            executionEnd
          }) => !executionEnd, true)(_ref7);
        })(_from);
      })(_merge) // @ts-ignore
      ), updateCache(_ref6) // @ts-ignore
      ), share()(_ref5));
      const analyzedTest = ( // @ts-ignore
      _merge2 = rxjs.merge((_testResult = testResult // @ts-ignore
      , filter(({
        preprocessEnd
      }) => preprocessEnd)(_testResult)), ( // @ts-ignore
      _of3 = rxjs.of(cachedTestFile) // @ts-ignore
      , filter(() => !hashDifference.size && testFile.hashes.size)(_of3))) // @ts-ignore
      , switchMap((testFile // @ts-ignore
      ) => {
        var _from2;

        return _from2 = from(testFile.tests) // @ts-ignore
        , mergeMap((_test // test hasn't changed and is in the cache
        ) => {
          var _ref9, _ref10;

          return _ref9 = (_ref10 = !hashDifference.size && cachedTestFile.tests.find(test => test.description === _test.description && 'executionEnd' in test) || // test has changed or wasn't in the cache
          // @ts-ignore
          runtimeProvider // @ts-ignore
          , test(testFile, _test)(_ref10) // @ts-ignore
          ), takeWhile(({
            executionEnd
          }) => !executionEnd, true)(_ref9);
        })(_from2);
      })(_merge2));
      const mergedResult = rxjs.merge(preprocessedTestFile, testResult, analyzedTest);
      return (// @ts-ignore
        _ref11 = (_mergedResult = mergedResult // @ts-ignore
        , operators.map(result => [cache, result])(_mergedResult) // @ts-ignore
        ), operators.takeUntil(unsubscribe)(_ref11)
      );
    }, [{}]) // @ts-ignore
    , operators.map(([, result]) => result)(_mergeScan)
  );
});

var Tester = (options => rxjs.Observable.create(observer => {
  var _ref, _Parcel, _ref2, _ref3, _ref4, _parcelBundle, _ref7, _from;
  ...
  _ref2 = (_ref3 = (_ref4 = (_parcelBundle = parcelBundle // @ts-ignore
  , operators.filter(({
    name
  }) => name === 'buildStart')(_parcelBundle) // @ts-ignore
    ...
    var _ref5, _ref6, _parcelBundle2;

    return (// @ts-ignore
      _ref5 = (_ref6 = (_parcelBundle2 = parcelBundle // @ts-ignore
      , operators.filter(({
        name
...
const run$1 = async (entries, command) => {
...
const watch = program.command('watch [input...]').description('starts the tester in watch mode').action(run$1);
applyOptions(watch, commonOptions);
const test$2 = program.command('test [input...]').description('test once').action(run$1);
applyOptions(test$2, commonOptions);
